function [wChange, vChange, qChange, piChange, lamChange, pChange, rChange, lChange, welfChange] = processCounterfactual(dist_mat_rail_ctf, dist_mat_highway_future, dni, psi, J, w_n, v_n, uncondCom, L_n, R_n, tradesh, outputDir, scenarioIndex)
    ...
    avDistChange_rail = (mean(dist_mat_rail_ctf, 2)) ./ (mean(dist_mat_rail, 2));
    avDistChange_highway = (mean(dist_mat_highway_future, 2)) ./ (mean(dist_mat_highway, 2));
    avDistChange = (mean(((dist_mat_rail_ctf.^0.09) .* (dist_mat_highway_future.^0.91)), 2)) ./ (mean(((dist_mat_rail.^0.09) .* (dist_mat_highway.^0.91)), 2));

    future_dni = readmatrix('data/output/MAP_fut_dni.csv');
    avdniChange = mean(dni_change, 2) ./ mean(dni, 2);
    % Save changes in average counterfactual accessibility
    headers_avDistChange = {sprintf('Average distance to other counties (scenario %d) - change relative to 2021', scenarioIndex)};
    headers_avDistChange_rail = {sprintf('Average rail distance to other counties (scenario %d) - change relative to 2021', scenarioIndex)};
    headers_avDistChange_highway = {sprintf('Average road distance to other counties (scenario %d) - change relative to 2021', scenarioIndex)};
    headers_avdniChange = {sprintf('Average change in commuting costs (scenario %d) - change relative to 2021', scenarioIndex)};
    writeMatrixWithHeader(avDistChange, headers_avDistChange, outputDir, sprintf('ctf%d_AvDist.csv', scenarioIndex));
    writeMatrixWithHeader(avDistChange_rail, headers_avDistChange, outputDir, sprintf('ctf%d_AvDistRail.csv', scenarioIndex));
    writeMatrixWithHeader(avDistChange_highway, headers_avDistChange, outputDir, sprintf('ctf%d_AvDistHighway.csv', scenarioIndex));
    writeMatrixWithHeader(avdniChange, headers_avdniChange, outputDir, sprintf('ctf%d_avdniChange.csv', scenarioIndex));
